...
def display_images(
    ...
    plt.axis("off")
    plt.subplot(2, 6, 3)
    plt.imshow(ce[ce_level, ...] - nce[nce_level, ...], cmap="bone", **ABDO_WINDOW)
    plt.axis("off")
    ...
    plt.axis("off")
    plt.subplot(2, 6, 6)
    plt.imshow(hq[hq_level, ...] - lq[1, ...], cmap="bone", **ABDO_WINDOW)
    plt.axis("off")
...
def main() -> None:
    ...
    with open(cwd / "source_coords.json") as fp:
    ...
    if arguments.to_include is None:
        ...
        subject_ids = [
            sub.stem[0:6]
            for sub in ce_path.glob("*")
            if sub.stem[0:6] in arguments.to_include.split(",")
        ]

    idx_start = (
        None if arguments.start_at is None else subject_ids.index(arguments.start_at)
    )
    idx_end = (
        None
        if arguments.stop_before is None
    ...
    for subject_id in subject_ids:
        ...
        for lq_name in lq_names:
            ...
            hq_candidates = sorted(
                hq_names,
                key=lambda x: abs(int(x[-3:]) - int(lq_name.stem[-3:])),
            )
