...
class VQBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        for dim in num_embeddings:
            self.dictionaries.append(
                self.add_weight(
                    f"VQdict_{dim}",
                    shape=[embedding_dim, dim],
    ...
    def call(self, x: tf.Tensor) -> tf.Tensor:
        ...
        weighted_dictionary_loss = tf.reduce_sum(
            dictionary_loss_tensor / self.num_dictionaries,
        )
        weighted_commitment_loss = tf.reduce_sum(
            commitment_loss_tensor / self.num_dictionaries,
        )
        ...
        weighted_q = tf.reduce_sum(
            tf.stack(q, axis=0) / self.num_dictionaries,
            axis=0,
        )
