...
class AffineTransform2D(AffineTransform):
    ...
    def get_img_indices(self) -> tuple[tf.Tensor, tuple[tf.Tensor]]:
        ...
        img_indices = tf.reshape(tf.range(self._mb_size) * self._height * self._width, [-1, 1])
        img_indices = tf.matmul(img_indices, tf.ones((1, self._height * self._width), dtype="int32"))
...
class AffineTransform3D(AffineTransform):
    ...
    def coord_gen(self) -> None:
        ...
        X, Y, Z = tf.meshgrid(tf.linspace(-1.0, 1.0, self.width_i), tf.linspace(-1.0, 1.0, self._height), tf.linspace(-1.0, 1.0, self._depth))
        flat_X = tf.reshape(X, (1, -1))
        flat_Y = tf.reshape(Y, (1, -1))
    ...
    def get_img_indices(self) -> tuple:
        ...
        # Creates a vector of base indices corresponding to each img in mb
        # Allows finding pixel in unrolled image vector
        img_indices = tf.reshape(tf.range(self._mb_size) * self._height * self._width * self._depth, [-1, 1])
        img_indices = tf.matmul(img_indices, tf.ones((1, self._height * self._width * self._depth), dtype="int32"))
        img_indices = tf.reshape(img_indices, [-1])
