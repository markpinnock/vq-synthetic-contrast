...
from vq_sce import MIN_HQ_DEPTH, RANDOM_SEED
...
class ContrastDataloader(BaseDataloader):
    """Contrast enhancement dataloader class."""
    ...
    def __init__(
        ...
        for t in self._targets.keys():
            ...
            for source_id in source_ids:
                ...
                ), f"Too many sources: {candidate_sources}"
    ...
    def _calc_coords(self, target_id: str) -> list[int]:
    ...
    def _generate_example_images(self) -> None:
        ...
        ex_sources_ids = []
        ex_targets_ids = np.random.choice(
    ...
    def data_generator(self) -> Iterator[DataDictType]:
        ...
        for target_id in self._target_ids[0 : self.N]:
            ...
            for _ in range(num_iter):
                ...
                data_dict: DataDictType = {"source": sub_source, "target": sub_target}

                yield data_dict

    def inference_generator(self) -> Iterator[dict[str, Any]]:
        for target_id in self._target_ids:
            source_candidates = fnmatch.filter(
                self.source_coords.keys(),
                f"{target_id[0:6]}HQ*",
            )
            target_candidates = fnmatch.filter(
                self.source_coords.keys(),
                f"{target_id[0:6]}AC*",
            )
            ...
            sort_by_closest = sorted(
                source_candidates,
                key=lambda x: abs(int(x[-3:]) - int(target_id[-3:])),
            )

            if (
                int(target_id[-3:]) < int(sort_by_closest[0][-3:])
            ...
            if source_id == self._target_source_map[target_id]:
                lower, upper = self._source_coords[target_id][
                    self._target_source_map[target_id]
                ]
            ...
            # If target is late CE image
                ...
                lower_1, upper_1 = self._source_coords[source_id][
                    self._target_source_map[target_id]
                ]
                lower_2, upper_2 = self._source_coords[target_id][
                    self._target_source_map[target_id]
                ]
                # I.e. source - target
                ...
                if lower >= 0 and upper <= 0:
                    ...
                    target = self._preprocess_image(
                        target,
                        lower,
                        lower + source.shape[0],
                    )
                elif lower >= 0 and upper > 0:
                    source = self._preprocess_image(
                        source,
                        None,
                        target.shape[0] - lower,
                    )
                    ...
                    target = self._preprocess_image(
                        target,
                        None,
                        source.shape[0] + lower,
                    )
                elif lower < 0 and upper > 0:
                    source = self._preprocess_image(
                        source,
                        -lower,
                        -lower + target.shape[0],
                    )
                    target = self._preprocess_image(target, None, None)
                else:
...
if __name__ == "__main__":
    ...
    for data in train_ds.batch(2).take(4):
        ...
        plt.subplot(3, 2, 1)
        plt.imshow(source[0, 11, :, :, 0], cmap="gray", vmin=-150, vmax=250)
        plt.axis("off")

        plt.subplot(3, 2, 2)
        plt.imshow(source[1, 11, :, :, 0], cmap="gray", vmin=-150, vmax=250)
        plt.axis("off")

        plt.subplot(3, 2, 3)
        plt.imshow(target[0, 11, :, :, 0], cmap="gray", vmin=-150, vmax=250)
        plt.axis("off")

        plt.subplot(3, 2, 4)
        plt.imshow(target[1, 11, :, :, 0], cmap="gray", vmin=-150, vmax=250)
        plt.axis("off")

        plt.subplot(3, 2, 5)
        plt.imshow(
            target[0, 11, :, :, 0] - source[0, 11, :, :, 0],
            cmap="gray",
            vmin=-150,
            vmax=250,
        )
