...
class TrainingLoop:
    ...
    def _save_train_results(self, epoch: int) -> None:
        ...
        self.results[f"train_{prefix}_total"].append(
            float(self.model.total_metric.result()),
        )
        ...
        if self.config["expt"]["log_scalars"]:
            with self.train_writer.as_default():
                tf.summary.scalar(
                    f"train_{prefix}_L1",
                    self.model.L1_metric.result(),
                    step=epoch,
                )
                tf.summary.scalar(
                    f"train_{prefix}_vq",
                    self.model.vq_metric.result(),
                    step=epoch,
                )
                tf.summary.scalar(
                    f"train_{prefix}_total",
                    self.model.total_metric.result(),
                    step=epoch,
                )
    ...
    def _save_val_results(self, epoch: int) -> None:
        ...
        self.results[f"val_{prefix}_total"].append(
            float(self.model.total_metric.result()),
        )
    ...
    def train(self, verbose: int = 1) -> None:
        ...
        if len(list(self.model_save_path.glob("model.ckpt*"))) > 0:
            ...
            self.results[f"train_{prefix}_L1"] = self.results[f"train_{prefix}_L1"][
                0:num_epochs_trained
            ]
            self.results[f"train_{prefix}_vq"] = self.results[f"train_{prefix}_vq"][
                0:num_epochs_trained
            ]
            self.results[f"train_{prefix}_total"] = self.results[
                f"train_{prefix}_total"
            ][0:num_epochs_trained]
            self.results[f"val_{prefix}_L1"] = self.results[f"val_{prefix}_L1"][
                0:num_epochs_trained
            ]
            self.results[f"val_{prefix}_vq"] = self.results[f"val_{prefix}_vq"][
                0:num_epochs_trained
            ]
            self.results[f"val_{prefix}_total"] = self.results[f"val_{prefix}_total"][
                0:num_epochs_trained
            ]
        ...
        for epoch in range(num_epochs_trained, self.epochs):
            ...
            if verbose:
                metric_results = [
                    metric.result().numpy() for metric in self.model.metrics
                ]
                print(  # noqa: T201
                    f"Train epoch {epoch + 1}, " f"L1, VQ, Total: {metric_results}",
                )
            ...
            if self.config["data"]["cv_folds"] > 1:
                ...
                if verbose:
                    ...
                    print(  # noqa: T201
                        f"Val epoch {epoch + 1}, " f"L1, VQ, Total: {metric_results}",
                    )
