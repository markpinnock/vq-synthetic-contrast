...
def test_generate_indices_2d(
    img_size: tuple[int, int, int], patch_size: list[int], strides: list[int]
) -> None:
    """Test indices of patches to be extracted in 2D."""

    def _setup_ground_truth(
        img: tf.Tensor, sizes: list[int], strides: list[int]
    ) -> tf.Tensor:
        ...
        patch = tf.reshape(patch, [tf.reduce_prod(patch.shape[1:3])] + sizes[1:3] + [1])
    ...
    assert np.equal(
        ground_truth.numpy().squeeze(), pred_indices.numpy().squeeze()
    ).all()
...
def test_extract_patches(
    ...
    assert np.equal(
        ground_truth.numpy().squeeze(), pred_patches.numpy().squeeze()
    ).all()
...
def test_CombinePatches(
    ...
    test_img = tf.cast(
        tf.reshape(tf.range(tf.reduce_prod(img_size)), img_size), "float32"
    )
    indices = generate_indices(test_img.shape, strides, patch_size)
